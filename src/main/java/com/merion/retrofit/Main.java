...
import com.merion.retrofit.response.JsonPlaceholderService;
...
public class Main {
    public  static  void  main(String[] arg) throws IOException {
        ...
        List<PostResponse> post =postResponses.body();
        System.out.println(post);

        System.out.println("POST:/posts ---------");
        PostCreateRequest createRequest=new PostCreateRequest();
        createRequest.setBody("create");
        createRequest.getTitle("create");
        createRequest.getUserId(1);
        PostResponse postResponseCreat =api.create(createRequest).execute().body();
        System.out.println(postResponseCreat);
